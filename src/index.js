...
const startApp = async () => {
  try {
    ...
    bot.use((ctx, next) => {
      if (
        (ctx.update.message && adminIds.includes(ctx.update.message.from.id.toString()))
        || (ctx.update.callback_query && adminIds.includes(ctx.update.callback_query.from.id.toString()))) {
    ...
    bot.action(/country_.*/, (ctx) => userController.selectCountry(ctx, bot));
